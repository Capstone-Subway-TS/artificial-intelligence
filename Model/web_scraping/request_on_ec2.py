...
def job(text):
    ...
    schedule.every(2).minutes.do(subway.fetch_subway_data)

# 매일 05:30에 첫 호출 시작
text = "API 시작"
schedule.every().day.at("15:00").do(job, text)

while True:
    # 현재 시간이 05:30 이후이고, 25:00 (익일 01:00) 이전인지 확인
    current_time = datetime.now()
    end_time = (datetime.now() + timedelta(days=1)).replace(hour=1, minute=0, second=0, microsecond=0)
    
    if current_time >= current_time.replace(hour=5, minute=30, second=0, microsecond=0) and current_time <= end_time:
