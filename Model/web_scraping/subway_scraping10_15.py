...
while True:
    # 현재 시간이 10:30 이후이고, 15:30 이전인지 확인
    current_time = datetime.now()
    end_time = current_time.replace(hour=15, minute=35, second=0, microsecond=0)
    
    if current_time >= current_time.replace(hour=10, minute=25, second=0, microsecond=0) and current_time <= end_time:
